...
void ObjHandle::update()
{
	if (!is_grabbed && change_pos && length2(speed) > UNIGINE_EPSILON)
	{
		...
		else
		{
			...
			speed = lerp(speed, vec3::ZERO, saturate(ACCELERATION_FACTOR * Game::get()->getIFps()));
		}
		if (body)
	...
	sound->setPitch(Game::get()->getScale());
}
...
void ObjHandle::grabIt(VRPlayer* player, int hand_num)
{
	SoundManager::get()->playSound(
		...
		1.0f,
		Game::get()->getRandomFloat(0.8f, 1.2f),
		sound_min_dist,
		sound_max_dist);
...
void ObjHandle::holdIt(VRPlayer* player, int hand_num)
{
	...
	if (change_pos) 
	{
		...
		speed = lerp(speed, target_speed, saturate(ACCELERATION_FACTOR * Game::get()->getIFps() / (length(target_speed) + 0.01f)));
		if (body)
			body->setLinearVelocity(speed);
		else
		...
		// slide sound
		if (length(handle_pos - last_handle_pos) / Game::get()->getIFps() >= 0.3f)
	...
	if (change_rot)
	{
		...
		Visualizer::get()->renderVector(node->getWorldPosition(), node->getWorldPosition() + old_rotate_vec, vec4::RED);
		Visualizer::get()->renderVector(node->getWorldPosition(), node->getWorldPosition() + new_rotate_vec, vec4::GREEN);
		Visualizer::get()->renderVector(node->getWorldPosition(), node->getWorldPosition() + up, vec4::BLUE);
		...
		if (angles.y != cangles.y) // TO DO: move axis selection to property
		{
			...
			if (!border_reached)
				...
				sound_stop,
				node->getWorldPosition(),
				Game::get()->getScale(), // volume
				Game::get()->getRandomFloat(0.9f, 1.1f),
				sound_min_dist,
				sound_max_dist);
	...
	if (slide_sound_timer > 0)
	{
		...
		slide_sound_timer = saturate(slide_sound_timer - Game::get()->getIFps());
	}
	else
	{
		sound->stop();
