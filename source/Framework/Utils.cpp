...
{
	if (Console::get()->getInt("show_visualizer") == 0)
		Console::get()->run("show_visualizer 1");

	Vec3 p = mat.getTranslate();
	Visualizer::get()->renderLine3D(p, p + Vec3(mat.m00, mat.m10, mat.m20) * scale, vec4(1, 0, 0, 1));
	Visualizer::get()->renderLine3D(p, p + Vec3(mat.m01, mat.m11, mat.m21) * scale, vec4(0, 1, 0, 1));
	Visualizer::get()->renderLine3D(p, p + Vec3(mat.m02, mat.m12, mat.m22) * scale, vec4(0, 0, 1, 1));
}
...
void visualizeBones(ObjectMeshSkinnedPtr &skin, float scale)
{
	...
	{
		Mat4 mat = skin->getWorldBoneTransform(i);
		Vec3 p = mat.getTranslate();
		Visualizer::get()->renderLine3D(p, p + Vec3(mat.m00, mat.m10, mat.m20) * scale, vec4(1, 0, 0, 1));
		Visualizer::get()->renderLine3D(p, p + Vec3(mat.m01, mat.m11, mat.m21) * scale, vec4(0, 1, 0, 1));
		Visualizer::get()->renderLine3D(p, p + Vec3(mat.m02, mat.m12, mat.m22) * scale, vec4(0, 0, 1, 1));
		Visualizer::get()->renderMessage3D(p, vec3(0, 0, 0), skin->getBoneName(i), vec4(1, 1, 1, 1), 1);
	}
}
