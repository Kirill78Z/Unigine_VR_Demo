...
int SoundManager::init()
{
	...
	Sound::get()->setAttenuation(Sound::ATTENUATION_INVERSE_CLAMPED);
	Sound::get()->setDoppler(0); // otherwise you can hear "beep" when moving in slowmo
	...
	for (int i = 0; i < queue.size(); i++)
	{
		...
		for (int k = 0; k < sounds.size(); k++)
		{
			if (!sounds[k].source->isPlaying())
			{
				...
				sounds[k].source->setPitch(queue[i].pitch * Game::get()->getScale());
		...
		if (need_new_sound)
		{
			...
			sound->setPitch(queue[i].pitch * Game::get()->getScale());
			sound->setLoop(0);
	...
	for (int i = 0; i < soundsLoop.size(); i++)
		if (soundsLoop[i].timeRemaining > 0)
		{
			...
			soundsLoop[i].source->setPitch(lerp(
				...
				soundsLoop[i].targetPitch * Game::get()->getScale(), 
				5.0f * Game::get()->getIFps() / Game::get()->getScale()));
	...
	for (int i = 0; i < queueLoop.size(); i++)
	{
		...
		for (int k = 0; k < soundsLoop.size(); k++)
		{
			if (soundsLoop[k].timeRemaining <= 0)
			{
				...
				soundsLoop[k].source->setPitch(queueLoop[i].pitch * Game::get()->getScale());
		...
		if (need_new_sound)
		{
			...
			sound->setPitch(queueLoop[i].pitch * Game::get()->getScale());
			sound->setLoop(1);
			sound->setOcclusion(0);
			sound->setSourceMask(SOUND_MASK);
			sound->play();
...
int SoundManager::shutdown()
{
	...
	for (int i = 0; i < sounds.size(); i++)
		sounds[i].source.destroy();
	sounds.clear();

	for (int i = 0; i < soundsLoop.size(); i++)
		soundsLoop[i].source.destroy();
	soundsLoop.clear();
...
Vector<String> SoundManager::getSoundGroup(const char *file_name)
{
	...
	for (const String &file : files)
		if (is_same(pattern.get(), file))
			result.append(fs->getAbsolutePath(file));
...
void SoundManager::playSound(const char *file_name, const Vec3 &pos, float volume, float pitch, float min_distance, float max_distance)
{
	...
	for (int i = 0; i < sounds.size(); i++)
		if (sounds[i].source->isPlaying() && strcmp(sounds[i].name.get(), file_name) == 0)
		{
			...
			if (sounds[i].source->getTime() < MIN_DELAY_BETWEEN_SAME_SOUNDS * Game::get()->getScale())
				return;
			...
			if (similar_sound_count >= SIMILAR_SOUND_LIMIT)
			{
				...
				Sound::get()->renderWorld(1);
				sounds[i].source->setSampleName(group == groups.end() ? pattern.get() : group->data[Game::get()->getRandomInt(0, group->data.size())].get());
				sounds[i].source->setPosition(pos);
				sounds[i].source->setGain(volume);
				sounds[i].source->setPitch(pitch * Game::get()->getScale());
	...
	if (group == groups.end() || group->data.size() == 0)
		...
		q.fileName = group->data[Game::get()->getRandomInt(0, group->data.size())];
	q.position = pos;
	q.volume = volume;
	q.pitch = pitch;
...
void SoundManager::playLoopSound(const char *file_name, const NodePtr &parent, float duration, float volume, float pitch, float min_distance, float max_distance)
{
	...
	auto group = groups.find(pattern);
	q.name = pattern;
	q.fileName = (group == groups.end() ? pattern : group->data[Game::get()->getRandomInt(0, group->data.size())]);
...
{
	return Sound::get()->getVolume();
}

void SoundManager::setSoundsVolume(float volume)
{
	Sound::get()->setSourceVolume(SOUND_CHANNEL, volume);
}

float SoundManager::getSoundVolume()
{
	return Sound::get()->getSourceVolume(SOUND_CHANNEL);
}

void SoundManager::setMusicVolume(float volume)
{
	Sound::get()->setSourceVolume(MUSIC_CHANNEL, volume);
}

float SoundManager::getMusicVolume()
{
	return Sound::get()->getSourceVolume(MUSIC_CHANNEL);
}
